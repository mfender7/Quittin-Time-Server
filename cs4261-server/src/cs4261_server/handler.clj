...
(defn search-recipes [param]
  (let [req (str base-search api-key "&allowedCourse[]=course^course-Dinner&maxTotalTimeInSeconds=3600&" param)]
    (let [resp (client/get req)]
      (let [body (json/read-str (get resp :body) :key-fn keyword)]
        (let [matches (:matches body)]
          (get-recipes (get (nth matches (rand-int 10)) :id)))))))
